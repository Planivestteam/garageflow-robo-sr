(() => {
  ...
  const AGENT_LABELS = {
    ...
    report: 'Report Agent',
    reengagement: 'Re-engagement Agent',
    'sms-outreach': 'SMS Outreach Agent',
  ...
  async function renderOverview(silent) {
    ...
    try {
      const [{ metrics, lastRuns, integrations }, today] = await Promise.all([
        api('/dashboard'),
        api('/today').catch(() => ({ meetingsToday: [], awaitingReply: [], staleDeals: [] })),
      ]);
      const m = metrics;
      const failedAgents = lastRuns.filter((r) => r.status === 'failed');
      const hasTodayItems = today.meetingsToday.length || today.awaitingReply.length || today.staleDeals.length;

      contentEl.innerHTML = `
        ${hasTodayItems ? `
        <div class="panel" style="border-left:3px solid var(--accent)">
          <div class="panel-header"><h2>📌 Hoje</h2><span class="muted">O que precisa da tua atenção agora</span></div>
          <div class="panel-body">
            ${today.meetingsToday.length ? `
              <div class="section-title">Reuniões hoje (${today.meetingsToday.length})</div>
              <div class="timeline">${today.meetingsToday.map((mt) => `
                <div class="timeline-item outbound clickable" data-open-lead="${mt.workshop_id}">
                  <div class="timeline-meta"><strong>${escapeHtml(mt.workshop_name)}</strong> · ${fmtDate(mt.scheduled_at)}</div>
                </div>
              `).join('')}</div>
            ` : ''}
            ${today.awaitingReply.length ? `
              <div class="section-title">Respostas por atender (${today.awaitingReply.length})</div>
              <div class="timeline">${today.awaitingReply.map((c) => `
                <div class="timeline-item inbound clickable" data-open-lead="${c.workshop_id}">
                  <div class="timeline-meta"><strong>${escapeHtml(c.workshop_name)}</strong> · ${escapeHtml(c.workshop_city || '')} ${c.classification ? `<span class="badge badge-accent">${escapeHtml(CLASSIFICATION_LABELS[c.classification] || c.classification)}</span>` : ''}</div>
                  <div class="timeline-body">${escapeHtml((c.body || '').slice(0, 140))}${(c.body || '').length > 140 ? '…' : ''}</div>
                </div>
              `).join('')}</div>
            ` : ''}
            ${today.staleDeals.length ? `
              <div class="section-title">Negócios parados há mais de 5 dias (${today.staleDeals.length})</div>
              <div class="timeline">${today.staleDeals.map((d) => `
                <div class="timeline-item clickable" data-open-lead="${d.workshop_id}">
                  <div class="timeline-meta"><strong>${escapeHtml(d.workshop_name)}</strong> · ${escapeHtml(d.workshop_city || '')} · desde ${fmtDate(d.updated_at)}</div>
                </div>
              `).join('')}</div>
            ` : ''}
          </div>
      ...
      $$('[data-open-lead]', contentEl).forEach((el) => {
        el.addEventListener('click', () => {
          window.location.hash = '#/leads';
          setTimeout(() => openLeadModal(el.dataset.openLead), 300);
        });
      });
    } catch (err) {
  ...
  async function renderLeads() {
    ...
    contentEl.innerHTML = `
      <div class="toolbar">
        <input type="text" id="leadSearchInput" placeholder="Pesquisar por nome ou cidade…" style="min-width:220px" />
        <select id="statusFilter">
          <option value="">Todos os estados</option>
          ${Object.entries(STATUS_LABELS).map(([k, v]) => `<option value="${k}">${v}</option>`).join('')}
        </select>
        <label style="display:flex;align-items:center;gap:6px;font-size:12.5px;color:var(--ink-muted)">
          <input type="checkbox" id="phoneOnlyFilter" /> Só telefone (sem email)
        </label>
        <button class="btn btn-sm" id="exportCsvBtn" type="button">⬇ Exportar CSV</button>
        <button class="btn btn-sm btn-primary" id="sendSelectedBtn" type="button" disabled>✉ Enviar aos selecionados (<span id="selectedCount">0</span>)</button>
      </div>
    ...
    $('#phoneOnlyFilter').addEventListener('change', renderLeadsTableFromCache);
    let searchDebounce;
    $('#leadSearchInput').addEventListener('input', () => {
      clearTimeout(searchDebounce);
      searchDebounce = setTimeout(renderLeadsTableFromCache, 200);
    });
    ...
    $('#sendSelectedBtn').addEventListener('click', async () => {
      ...
      if (!confirm(`Enviar o próximo email da sequência a ${selectedLeadIds.size} oficina(s) selecionada(s)?`)) return;
      const btn = $('#sendSelectedBtn');
      btn.disabled = true;
      btn.textContent = 'A enviar…';
      try {
        const result = await api('/outreach/send-selected', {
          method: 'POST',
          body: JSON.stringify({ workshopIds: [...selectedLeadIds] }),
        });
        alert(`${result.sent} de ${result.total} enviados com sucesso. Os restantes foram ignorados (sem email, sequência já terminada, ou ainda dentro do intervalo de follow-up).`);
        loadLeadsTable();
      } catch (err) {
        alert('Erro: ' + err.message);
      } finally {
        btn.disabled = selectedLeadIds.size === 0;
        btn.innerHTML = `✉ Enviar aos selecionados (<span id="selectedCount">${selectedLeadIds.size}</span>)`;
      }
    });
    loadLeadsTable();
  }
  ...
  function updateSendSelectedButton() {
    ...
    if (countEl) countEl.textContent = String(selectedLeadIds.size);
  }
  ...
  async function loadLeadsTable() {
    ...
    try {
      ...
      leadsCache = workshops;
      renderLeadsTableFromCache();
    } catch (err) {
      wrap.innerHTML = errorPanel(err);
    }
  }

  function renderLeadsTableFromCache() {
    const wrap = $('#leadsTableWrap');
    const query = ($('#leadSearchInput')?.value || '').trim().toLowerCase();
    const phoneOnly = $('#phoneOnlyFilter')?.checked;
    let workshops = query
      ? leadsCache.filter((w) => w.name.toLowerCase().includes(query) || (w.city || '').toLowerCase().includes(query))
      : leadsCache;
    if (phoneOnly) {
      workshops = workshops.filter((w) => w.phone && !w.email);
    }

    if (!workshops.length) {
      wrap.innerHTML = emptyState(query ? 'Nenhuma oficina corresponde à pesquisa.' : 'Sem oficinas para este filtro ainda.');
      return;
    }
    wrap.innerHTML = `
      <table>
        <thead><tr><th></th><th>Oficina</th><th>Cidade</th><th>Telefone</th><th>Email</th><th>Score</th><th>Estado</th><th>Ações</th></tr></thead>
        <tbody>
          ${workshops.map((w) => `
            <tr data-id="${w.id}">
              <td style="width:32px"><input type="checkbox" class="lead-checkbox" data-id="${w.id}" ${selectedLeadIds.has(w.id) ? 'checked' : ''} /></td>
              <td class="clickable" style="cursor:pointer">${escapeHtml(w.name)}${w.demo_mode ? ' <span class="badge badge-neutral">demo</span>' : ''}</td>
              ...
              <td style="white-space:nowrap">
                ${w.phone ? `<button class="btn btn-sm quick-call" data-id="${w.id}" title="Ligar">📞</button> <button class="btn btn-sm quick-whatsapp" data-id="${w.id}" title="WhatsApp">💬</button>` : ''}
              </td>
            </tr>
          `).join('')}
        </tbody>
    ...
    $$('td.clickable', wrap).forEach((cell) => {
      cell.addEventListener('click', () => openLeadModal(cell.closest('tr').dataset.id));
    });
    $$('.quick-call', wrap).forEach((btn) => {
      btn.addEventListener('click', async (e) => {
        e.stopPropagation();
        try {
          const { telLink } = await api(`/workshops/${btn.dataset.id}/call-link`);
          window.location.href = telLink;
          await api(`/workshops/${btn.dataset.id}/call-logged`, { method: 'POST', body: '{}' });
        } catch (err) {
          alert('Erro: ' + err.message);
        }
      });
    });
    $$('.quick-whatsapp', wrap).forEach((btn) => {
      btn.addEventListener('click', async (e) => {
        e.stopPropagation();
        try {
          const { link, message } = await api(`/workshops/${btn.dataset.id}/whatsapp-link`);
          window.open(link, '_blank', 'noopener');
          await api(`/workshops/${btn.dataset.id}/whatsapp-logged`, { method: 'POST', body: JSON.stringify({ message }) });
        } catch (err) {
          alert('Erro: ' + err.message);
        }
      });
  ...
  async function openLeadModal(id) {
    ...
    try {
      ...
      const html = `
        ...
        <div class="modal-body">
          ...
          <dl class="kv-grid">
            ...
            <dt>Telefone</dt><dd class="mono">${escapeHtml(w.phone || '—')} ${w.phone ? '<button class="btn btn-sm" id="callBtn" type="button" style="margin-left:8px">📞 Ligar</button> <button class="btn btn-sm" id="whatsappBtn" type="button" style="margin-left:6px">💬 Abrir no WhatsApp</button>' : ''}</dd>
            <dt>Email</dt><dd class="mono">${escapeHtml(w.email || '—')} ${w.email_bounced ? '<span class="badge badge-danger">inválido -- nunca mais será contactado por email</span>' : ''}</dd>
          ...
          <div class="section-title">As tuas notas</div>
          <div class="field">
            <textarea id="manualNotesInput" rows="3" placeholder="Ex: não atendeu, tentar depois das 14h...">${escapeHtml(w.manual_notes || '')}</textarea>
          </div>
          <button class="btn btn-sm" id="saveNotesBtn" type="button">💾 Guardar notas</button>
          ...
          ${conversations.length ? `<div class="timeline">${conversations.map((c) => `
            <div class="timeline-item ${c.direction}">
              <div class="timeline-meta">
                ${c.direction === 'inbound' ? 'Recebido' : (c.channel === 'whatsapp' ? 'Enviado (WhatsApp)' : c.channel === 'phone' ? 'Chamada telefónica' : 'Enviado (email)')} · ${fmtDate(c.created_at)}
          ...
          ${w.email ? `
            ...
            <button class="btn btn-sm" id="suggestReplyBtn" type="button">🤖 Sugerir resposta com IA</button>
      ...
      $('#saveNotesBtn').addEventListener('click', async () => {
        ...
        try {
          await api(`/workshops/${w.id}/notes`, { method: 'POST', body: JSON.stringify({ notes: $('#manualNotesInput').value }) });
          btn.textContent = '✓ Guardado';
          setTimeout(() => { btn.textContent = '💾 Guardar notas'; btn.disabled = false; }, 1500);
        } catch (err) {
      ...
      if (callBtn) {
        callBtn.addEventListener('click', async () => {
          try {
            const { telLink } = await api(`/workshops/${w.id}/call-link`);
            window.location.href = telLink;
            // Regista a tentativa de chamada na conversa, para ficar
            // visivel no historico (a chamada em si e feita por ti).
            await api(`/workshops/${w.id}/call-logged`, { method: 'POST', body: '{}' });
          } catch (err) {
            alert('Erro ao preparar a chamada: ' + err.message);
          }
        });
      }
      ...
      if (suggestReplyBtn) {
        suggestReplyBtn.addEventListener('click', async () => {
          ...
          suggestReplyBtn.textContent = 'A pensar…';
          try {
            const { draft } = await api(`/workshops/${w.id}/suggest-reply`);
            $('#replyBodyInput').value = draft;
          } catch (err) {
            alert('Erro ao sugerir resposta: ' + err.message);
          } finally {
            suggestReplyBtn.disabled = false;
            suggestReplyBtn.textContent = '🤖 Sugerir resposta com IA';
          }
        });
      }
      if (sendReplyBtn) {
  ...
  // ---------------------------------------------------------------------
  // ---------------------------------------------------------------------
  // PAGE: History (dia / semana / mes / ano)
  // ---------------------------------------------------------------------
  async function renderHistory() {
    setTitle('Histórico', 'Evolução das métricas por dia, semana, mês e ano.');
    contentEl.innerHTML = `
      <div class="toolbar">
      ...
      <div id="historyWrap" class="panel"><div class="loading">A carregar…</div></div>
    `;
    $('#historyPeriod').addEventListener('change', renderHistoryTable);
  ...
  async function renderHistoryTable() {
    ...
    try {
      ...
      const rows = [...(data[period] || [])].reverse();

      if (!rows.length) {
        wrap.innerHTML = emptyState('Ainda não há relatórios diários guardados suficientes para mostrar histórico. O Report Agent tem de correr pelo menos uma vez (às 20h UTC).');
        return;
      }

      wrap.innerHTML = `
        <table>
          <thead><tr><th>Período</th><th>Novas oficinas</th><th>Emails enviados</th><th>Respostas</th><th>Reuniões</th><th>Clientes ganhos</th></tr></thead>
          <tbody>
            ${rows.map((r) => `
              <tr>
                <td class="mono">${escapeHtml(r.period)}</td>
                <td class="mono">${r.newWorkshops}</td>
                <td class="mono">${r.emailsSent}</td>
                <td class="mono">${r.replies}</td>
                <td class="mono">${r.meetingsScheduled}</td>
                <td class="mono">${r.dealsWon}</td>
              </tr>
            `).join('')}
          </tbody>
        </table>
