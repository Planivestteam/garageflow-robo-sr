...
export const workshopsRepo = {
  ...
  markEmailBouncedByAddress(email) {
    const result = db.prepare('UPDATE workshops SET email_bounced = 1, updated_at = datetime(\'now\') WHERE lower(email) = ?').run(email.toLowerCase());
    return result.changes;
  },
  update(id, fields) {
  ...
  listQualifiedForOutreach(limit = 200) {
    return db.prepare("SELECT * FROM workshops WHERE status = 'qualified' AND email IS NOT NULL AND unsubscribed = 0 AND email_bounced = 0 ORDER BY score DESC LIMIT ?").all(limit);
  },
  listQualifiedWithoutEmail(limit = 200) {
    return db.prepare("SELECT * FROM workshops WHERE status = 'qualified' AND (email IS NULL OR email = '') ORDER BY score DESC LIMIT ?").all(limit);
  },
  listForSmsOutreach(limit = 200) {
    // Oficinas qualificadas so por telefone (sem email), que ainda nao
    // receberam SMS, e nao cancelaram subscricao.
    return db.prepare(`
      SELECT * FROM workshops
      WHERE status = 'qualified' AND (email IS NULL OR email = '')
      AND phone IS NOT NULL AND sms_sent_at IS NULL AND unsubscribed = 0
  ...
  listForReengagement(days = 90, limit = 200) {
    // Oficinas que terminaram a sequencia de outreach sem responder,
    // ha mais de X dias, e que ainda nao foram reativadas nenhuma vez.
    return db.prepare(`
      SELECT * FROM workshops
      WHERE status = 'sequence_completed'
      AND reengaged_at IS NULL
      AND id IN (
        SELECT workshop_id FROM outreach_emails
        WHERE status = 'sent'
        GROUP BY workshop_id
        HAVING MAX(sent_at) < datetime('now', '-' || ? || ' days')
      )
      LIMIT ?
    `).all(days, limit);
  },
...
export const outreachRepo = {
  ...
  countStepsForWorkshop(workshopId, sinceIso = null) {
    if (sinceIso) {
      return db.prepare("SELECT sequence_step FROM outreach_emails WHERE workshop_id = ? AND status = 'sent' AND sent_at > ?").all(workshopId, sinceIso);
    }
...
export const conversationsRepo = {
  ...
  listAwaitingReply(limit = 50) {
    // Oficinas cuja ultima mensagem na conversa e "inbound" (recebida) e
    // ainda nao teve nenhuma resposta outbound depois -- ou seja, estao
    // mesmo a espera de resposta humana.
    return db.prepare(`
      SELECT c.*, w.name as workshop_name, w.city as workshop_city
      FROM conversations c
      JOIN workshops w ON w.id = c.workshop_id
      WHERE c.direction = 'inbound'
      AND c.created_at = (
        SELECT MAX(c2.created_at) FROM conversations c2 WHERE c2.workshop_id = c.workshop_id
      )
      AND c.classification != 'nao_interessado'
...
export const meetingsRepo = {
  ...
  listToday() {
    return db.prepare(`
      SELECT m.*, w.name as workshop_name, w.phone as workshop_phone, w.email as workshop_email
      FROM meetings m JOIN workshops w ON w.id = m.workshop_id
      WHERE date(m.scheduled_at) = date('now') AND m.status != 'canceled'
...
export const dealsRepo = {
  ...
  listStale(days = 5) {
    return db.prepare(`
      SELECT d.*, w.name as workshop_name, w.city as workshop_city
      FROM deals d JOIN workshops w ON w.id = d.workshop_id
      WHERE d.stage = 'demo_done' AND datetime(d.updated_at) < datetime('now', '-' || ? || ' days')
...
export const prospectingTargetsRepo = {
  ...
  expandCoverageIfComplete(allConcelhos, batchSize = 20) {
    const nothingPending = this.nextBatch(1).length === 0;
    if (!nothingPending) return 0;

    const existingNames = new Set(
      db.prepare('SELECT name FROM prospecting_targets').all().map((r) => r.name)
    );
    const missing = allConcelhos.filter((c) => !existingNames.has(c.name));
    if (!missing.length) return 0; // pais inteiro ja coberto, nada mais a expandir

    const toAdd = missing.slice(0, batchSize);
    const insert = db.prepare('INSERT INTO prospecting_targets (id, name, distrito, status) VALUES (?, ?, ?, ?)');
    const tx = db.transaction((rows) => {
      for (const row of rows) insert.run(uuidv4(), row.name, row.distrito, 'pending');
    });
    tx(toAdd);
    return toAdd.length;
  },
  ...
  nextBatch(limit = 8) {
    // Prioriza: 1) concelhos nunca tentados; 2) falhados ha mais de 3
    // dias; 3) ja concluidos ha mais de 90 dias -- para o sistema NUNCA
    // parar de procurar oficinas novas (negocios abrem e sao mapeados
    // continuamente, mesmo depois de um concelho ja ter sido coberto).
    return db.prepare(`
      SELECT * FROM prospecting_targets
      WHERE status = 'pending'
         OR (status = 'failed' AND (last_attempt_at IS NULL OR last_attempt_at < datetime('now', '-3 days')))
         OR (status = 'done' AND last_attempt_at < datetime('now', '-90 days'))
...
export const alertsRepo = {
  shouldAlert(alertKey, cooldownHours = 24) {
    const last = db.prepare('SELECT sent_at FROM system_alerts WHERE alert_key = ? ORDER BY sent_at DESC LIMIT 1').get(alertKey);
    if (!last) return true;
    const hoursSince = (Date.now() - new Date(`${last.sent_at}Z`).getTime()) / (1000 * 60 * 60);
    return hoursSince >= cooldownHours;
  },
  recordAlert(alertKey) {
    db.prepare('INSERT INTO system_alerts (id, alert_key) VALUES (?, ?)').run(uuidv4(), alertKey);
  },
};
...
export const reportsRepo = {
  ...
  listAllWithMetrics(limit = 400) {
    return db.prepare('SELECT report_date, metrics_json FROM daily_reports ORDER BY report_date ASC LIMIT ?').all(limit);
  },
