  ...
  try {
    ...
    for (const workshop of pending) {
      try {
        ...
        // Ainda sem email: tenta a pagina de Facebook, se conhecida
        // (secao "Sobre" de paginas de negocio publicas por vezes
        // mostra o email sem precisar de login).
        if (!email && facebook) {
          const scrapedFb = await scrapePublicWebsite(facebook, logger);
          if (scrapedFb.emails.length) {
            email = scrapedFb.emails[0];
            summary.emailsFound += 1;
          }
          await sleep(1000);
        }
