...
import { agentRunsRepo, alertsRepo } from '../db/repositories.js';
...
import { runReengagementAgent } from './reengagementAgent.js';
import { runSmsOutreachAgent } from './smsOutreachAgent.js';
import { runDeduplicationAgent } from './deduplicationAgent.js';
...
const AGENTS = {
  ...
  reengagement: runReengagementAgent,
  'sms-outreach': runSmsOutreachAgent,
  deduplication: runDeduplicationAgent,
};
...
export async function runFullPipeline() {
  ...
  const pipelineOrder = ['prospecting', 'enrichment', 'deduplication', 'contact-hunter', 'qualification', 'outreach', 'sms-outreach', 'conversation', 'booking', 'conversion', 'analytics', 'report', 'reengagement'];
  const results = {};
...
export async function runHealthCheck() {
  ...
  for (const run of lastRuns) {
    ...
    if (run.status === 'failed') {
      alerts.push({ key: `failed:${run.agent_name}`, text: `Agente "${run.agent_name}" -- ultima execucao FALHOU as ${run.started_at}. Erro: ${run.error || 'desconhecido'}` });
    }
    if (ageHours > 30) {
      alerts.push({ key: `stale:${run.agent_name}`, text: `Agente "${run.agent_name}" nao corre ha mais de 30 horas (ultima execucao: ${run.started_at}).` });
    }
  }

  if (alerts.length) {
    alerts.forEach((a) => logger.warn(`CEO Agent healthcheck: ${a.text}`));

    // Envia um email de alerta, mas no maximo uma vez a cada 24h por
    // cada problema especifico -- para nunca inundar a caixa de correio
    // com o mesmo aviso repetido a cada 10 minutos.
    const toAlert = alerts.filter((a) => alertsRepo.shouldAlert(a.key, 24));
    if (toAlert.length) {
      try {
        const html = `<div style="font-family:Arial,sans-serif;font-size:15px;color:#222">
          <h2 style="color:#c4432b">⚠ GarageFlow -- problemas detetados</h2>
          <ul>${toAlert.map((a) => `<li>${a.text}</li>`).join('')}</ul>
          <p style="color:#888;font-size:12px">Este alerta so e reenviado uma vez a cada 24h por cada problema, para nao inundar a caixa de correio.</p>
        </div>`;
        await sendEmail({
          to: config.report.recipients[0],
          bcc: config.report.recipients.slice(1),
          subject: `⚠ GarageFlow -- ${toAlert.length} problema(s) detetado(s)`,
          html,
          text: toAlert.map((a) => a.text).join('\n'),
        }, logger);
        toAlert.forEach((a) => alertsRepo.recordAlert(a.key));
        logger.info(`Alerta enviado por email para ${toAlert.length} problema(s) novo(s)/nao confirmado(s) nas ultimas 24h.`);
      } catch (err) {
        logger.error(`Falha ao enviar email de alerta: ${err.message}`);
      }
    }
  } else {
  ...
  return { healthy: alerts.length === 0, alerts: alerts.map((a) => a.text), checkedAt: new Date().toISOString() };
}
